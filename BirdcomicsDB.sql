...
CREATE TABLE IF NOT EXISTS `shopping-cart`.`product` (
  ...
  `active` TINYINT(1) NOT NULL DEFAULT 1, -- New field added for active status
...
CREATE TABLE IF NOT EXISTS `shopping-cart`.`orders` (
  ...
  CONSTRAINT `productid`
    ...
    -- ON DELETE CASCADE -- Rimuovi questa linea
    ON UPDATE NO ACTION
...
CREATE TABLE IF NOT EXISTS `shopping-cart`.`user` (
  ...
  `usertype` ENUM('admin', 'guest') NOT NULL DEFAULT 'guest',  -- Aggiunto campo usertype
  PRIMARY KEY (`email`)
) ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS `shopping-cart`.`transactions` (
  ...
  CONSTRAINT `truserid`
    ...
    -- ON DELETE NO ACTION -- Rimuovi questa linea
    ON UPDATE NO ACTION,
...
INSERT INTO `shopping-cart`.`user` (`email`, `name`, `mobile`, `address`, `pincode`, `password`, `usertype`)
VALUES ('guest@gmail.com', 'Guest User', 9876543234, 'K.P Road, Gaya, Bihar - India', 879767, 'guest', 'guest');

-- Inserimento dell'utente admin
INSERT INTO `shopping-cart`.`user` (`email`, `name`, `mobile`, `address`, `pincode`, `password`, `usertype`)
VALUES ('admin@gmail.com', 'Admin User', 9876543459, 'ABC Colony, Newtown, West Bengal', 786890, 'admin', 'admin');
